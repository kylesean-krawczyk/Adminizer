-- ============================================================================
-- COMPREHENSIVE DATABASE FIX SCRIPT V3
-- Fixes: 500 errors on user_profiles queries, missing columns, RLS issues
-- Fixed: Organizations INSERT with proper array syntax
-- ============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Check if organizations table has enabled_verticals column
    SELECT EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'organizations'
        AND column_name = 'enabled_verticals'
    ) INTO has_enabled_verticals;
    ...
    IF NOT admin_profile_exists THEN
      ...
      IF admin_org_id IS NULL THEN
        IF has_enabled_verticals THEN
          -- Organizations table has enabled_verticals column
          INSERT INTO organizations (name, created_by, enabled_verticals)
          VALUES ('Master Organization', admin_auth_id, '{church,business,estate}')
          RETURNING id INTO admin_org_id;
        ELSE
          -- Organizations table doesn't have enabled_verticals column
          INSERT INTO organizations (name, created_by)
          VALUES ('Master Organization', admin_auth_id)
          RETURNING id INTO admin_org_id;
        END IF;

...
import { StatCard, DepartmentFeature, DepartmentTool, DepartmentIntegration, QuickAction } from '../../../types/departmentLandingPage'
...
type EditorMode = 'stat-card' | 'feature' | 'tool' | 'integration' | 'quick-action' | null
type EditingItem = StatCard | DepartmentFeature | DepartmentTool | DepartmentIntegration | QuickAction | null
...
export const DepartmentLandingCustomization: React.FC<DepartmentLandingCustomizationProps> = ({
  ...
  const [localIntegrations, setLocalIntegrations] = useState<DepartmentIntegration[]>([])
  const [localQuickActions, setLocalQuickActions] = useState<QuickAction[]>([])
  ...
  useEffect(() => {
    setLocalQuickActions(quickActions)
  }, [quickActions])
  ...
  const handleEditIntegration = (integration: DepartmentIntegration) => {
  ...
  const handleToggleIntegrationVisibility = async (id: string, isEnabled: boolean) => {
    await supabase
  ...
  const handleReorderIntegrations = async (newIntegrations: DepartmentIntegration[]) => {
    ...
    await reorderIntegrations({
      ...
      verticalId: vertical.id,
      departmentId,
      updates: newIntegrations.map((integration, idx) => ({
        id: integration.id,
        display_order: idx
      }))
    })
  ...
  const handleSaveQuickAction = async (data: any) => {
    if (!user?.organizationId) return
  ...
  const handleDeleteQuickAction = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this quick action?')) return
    await deleteQuickAction(id)
    await refetch()
  }

  const handleToggleQuickActionVisibility = async (id: string, isEnabled: boolean) => {
    await supabase
      .from('quick_actions')
      .update({ is_enabled: isEnabled })
      .eq('id', id)
  ...
  const handleReorderQuickActions = async (newActions: QuickAction[]) => {
    setLocalQuickActions(newActions)
    ...
    await reorderQuickActions({
      ...
      verticalId: vertical.id,
      departmentId,
      updates: newActions.map((action, idx) => ({
        id: action.id,
        display_order: idx
      }))
    })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-gray-200 rounded-lg">
        ...
        {expandedSections.integrations && (
          <div className="p-4 border-t border-gray-200 space-y-4">
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-600">
                Third-party platform integrations with OAuth support
              </p>
              <button
            ...
            <DraggableIntegrationList
              ...
              onReorder={handleReorderIntegrations}
              onEdit={handleEditIntegration}
              onDelete={handleDeleteIntegration}
              onToggleVisibility={handleToggleIntegrationVisibility}
            />
          </div>
        )}
      </div>
      ...
      <div className="border border-gray-200 rounded-lg">
        ...
        {expandedSections.quickActions && (
          <div className="p-4 border-t border-gray-200 space-y-4">
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-600">
                Quick action buttons displayed at the top of the department page
              </p>
              <button
            ...
            <DraggableQuickActionList
              ...
              onEdit={handleEditQuickAction}
              onDelete={handleDeleteQuickAction}
              onToggleVisibility={handleToggleQuickActionVisibility}
            />
          </div>
        )}
      </div>
      ...
      {editorMode === 'integration' && (
        <IntegrationEditorModal
          integration={editingItem as DepartmentIntegration}
          onSave={handleSaveIntegration}
          onCancel={() => {
            setEditorMode(null)
            setEditingItem(null)
          }}
          existingOrders={localIntegrations.map(i => i.display_order)}
        />
      )}

      {editorMode === 'quick-action' && (
        <QuickActionEditorModal
          quickAction={editingItem as QuickAction}
          integrations={localIntegrations}
          ...
          existingOrders={localQuickActions.map(a => a.display_order)}
        />
      )}
    </div>
  )
